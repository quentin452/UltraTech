				...
				if(FluidContainerRegistry.isContainer(craft.getStackInSlot(c))){
					...
					if(f != null){
						for(TankConection tan : tanks){
							int toD = 1000;
							if(liq.containsKey(f.getFluid())){
								toD = liq.get(f.getFluidID())+1000;
							}
							FluidStack drain = tan.tank.drain(tan.side, toD, false);
							if(drain != null && drain.getFluidID() == f.getFluidID() && drain.amount >= toD){
								found[c] = true;
								liq.put(f.getFluidID(),toD);
								break;
							}
						}
					}
						...
						if(FluidContainerRegistry.isContainer(craft.getStackInSlot(c))){
							...
							if(f != null){
								for(TankConection tan : tanks){
									...
									if(drain != null && drain.getFluidID() == f.getFluidID() && drain.amount >= 1000){
